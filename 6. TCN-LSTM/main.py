...
logging.basicConfig(filename='log.log',         # 输出到文件 demo.log中
                    filemode='a',               # 先清空再写入模式
                    level=logging.INFO,         # 设置日志级别
                    format="%(asctime)s--%(lineno)d-%(message)s",    # 设置日志格式
                    datefmt= "%Y-%m-%d  %H:%M:%S %a")       # 配置输出时间的格式
...
class MyMetrics:
    ...
    def print_metrics(self):
        for key, values in self.metrics.items():
            ...
            logging.info(f"The {key} is:{values}.")
...
class Train:
    ...
    def _prepare_data(self):
        ...
        test_generator = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=1024, shuffle=False)
    ...
    def train_model(self):
        ...
        with alive_bar(total=self.N_EPOCHS, title='training') as bar:
            for epoch in range(self.N_EPOCHS):
                ...
                logging.info(f'\tTrain MSE_Loss: {train_loss:.4f}')
                logging.info(f'\t Val. MSE_Loss: {valid_loss:.4f}')
    ...
    def predict(self):
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(self.test_generator):
                x = x.to(DEVICE)  # [batch_size, seq_len, feature]
                y = y.to(DEVICE)  # [batch_size, predict_seqlen]
                y_hat = self.model(x).squeeze(dim=2)  # y_hat [batch_size, predict_seqlen, 1]
...
def main():
    ...
    # 模型 优化器 损失函数
    model = seq2seq.Seq2Seq_TCN(input_size=HyperParams['features'],
                            encoder_num_channels = [4,6,8],
    ...
    filename = f"{HyperParams['datafile']}-{HyperParams['input_seqlen']} to {HyperParams['predict_seqlen']}"
